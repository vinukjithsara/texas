...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-white/[0.06] bg-surface">
      <Container>
        <Reveal className="grid gap-12 py-16 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8">
            ...
            <div className="mt-6 flex items-center gap-3">
              {SOCIALS.map(({ id, label, href, icon: Icon }) => (
                <a
                  ...
                  rel="noreferrer"
                  aria-label={label}
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 text-cream/70 transition-all duration-300 hover:-translate-y-0.5 hover:scale-110 hover:border-ember-500/40 hover:text-ember-400"
                >
                  <Icon size={17} />
                </a>
              ))}
          ...
          <div>
            ...
            <ul className="mt-5 space-y-3 text-sm text-muted">
              ...
              <li className="flex items-center gap-2.5">
                ...
                <a href={`mailto:${SITE.contact.email}`} className="break-all hover:text-ember-400">
                  {SITE.contact.email}
                </a>
              </li>
            </ul>
          ...
          <div>
            ...
            <ul className="mt-5 space-y-3 text-sm text-muted">
              {SITE.hours.map((h) => (
                <li key={h.day} className="flex flex-wrap justify-between gap-x-3 gap-y-0.5">
                  <span>{h.day}</span>
